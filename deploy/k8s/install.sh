...
kubectl exec -it $(kubectl get pod -l "app=api" -o jsonpath='{.items[0].metadata.name}') -- /bin/sh -c "curl --location --request POST 'http://localhost:4000/register' --header 'Content-Type: application/json' --data-raw '{ \"org\": \"${ORG}\", \"userId\": \"${USER_ID}\" }'"
}
